...
do
  ...
  if [ "${nsmark}" == "ns" ]; then
     ...
     echo "${name}" >> ${curdir}/Analysis/${prefix}.sexpre
  else
...
for file in ${curdir}/Analysis/*sexpre
do 
  name=$(basename "${file}" | cut -d "." -f 1)
  cat ${file} | sort > ${curdir}/Analysis/${name}.sex
  rm -rf ${file}
done
...
if [ -d "${curdir}/untgn_aln" ]; then  
   for file in ${curdir}/untgn_aln/*phy
   do 
   ...
   cat ${curdir}/Analysis/A.auto ${curdir}/Analysis/*sex > ${curdir}/Analysis/order2
  
   rm -rf ${curdir}/Analysis/A.auto ${curdir}/Analysis/*sex

   cat ${curdir}/Analysis/order ${curdir}/Analysis/order2 > ${curdir}/Analysis/orderfinal

   rm -rf ${curdir}/Analysis/order ${curdir}/Analysis/order2
   mv ${curdir}/Analysis/orderfinal ${curdir}/Analysis/order
fi
