...
usage()
{
 echo "Usage: [-a] <Alignment folder> [-s] <concatenated sequence for species tree>"
}
...
read -p "
 Files checklist: 1.Alignment folder [-a]
                  2.Concatenated sequence for species tree [-s]
                  3.get_cluster.R
                  4.logLkhforR_make.R
                  5.graphs.R
                  6.find_potent_knum.R
                  7.clusterAssign.R
...
if [ "${answer}" == "Y" -o "${answer}" == "y" ]; then
   if [ -z "${alnf}" ] || [ ! -d "${curdir}/${alnf}" ] || [ -z "${sp}" ] || [ ! -e ${curdir}/${sp} ] || [ ! -e get_cluster.R ] || [ ! -e logLkhforR_make.R ] || [ ! -e graphs.R ] || [ ! -e find_potent_knum.R ] || [ ! -e clusterAssign.R ]; then echo -e "\nMsg: Some files are missing.\n" && usage && exit; fi  #Check if all the files exist 
...
mv ${curdir}/${sp} ${curdir}/${alnf}/species.phy
...
mv ${curdir}/${alnf}/species.phy ${curdir}/species.phy
...
sexyn="n"
echo "species" > ${curdir}/Analysis/A.auto
sexi=1
...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   mv ${curdir}/species.phy ${curdir}/${alnf}/species.phy
   for alnfile in ${curdir}/${alnf}/*phy
   do
