...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   do
     ...
     nwt=$(cat gnTrees_collection.tre | sed -n "${spi}p")     
   ...
   do
     name=$(basename ${bst} | cut -d "." -f 1 | cut -d "-" -f 2)
     trel=$(cat ${bst} | wc -l)     
     trli=1
     while [ "${trli}" -le "${trel}" ]
     do
       lcon=$(sed -n "${trli}p" ${bst})
       if [ "${trli}" -lt 10 ]; then
          sed -i "${trli}s/${lcon}/${name}-00${trli} ${lcon}/" ${bst}
       elif [ "${trli}" -lt 100 ]; then
          sed -i "${trli}s/${lcon}/${name}-0${trli} ${lcon}/" ${bst}
       else
          sed -i "${trli}s/${lcon}/${name}-${trli} ${lcon}/" ${bst}
       fi
       ((trli++))
     done
   done
