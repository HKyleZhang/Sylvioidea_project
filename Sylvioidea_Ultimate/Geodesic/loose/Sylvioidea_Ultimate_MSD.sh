...
mv ${curdir}/${fdname}/bs-file ${curdir}/.bs-file
rm -rf ${curdir}/${fdname}/
...
#2.Recalculate the bootstrapping values
#*************************************************************
echo "
         Part 2. Recalculation of Bootstrapping values"
...
#Recalculation
rm -rf ${curdir}/.bs-file/ ${curdir}/${fdname}_bsrecal/
mkdir ${curdir}/${fdname}_bsrecal/

for file in ${curdir}/${fdname}_rooted/*treefile
do 
  namepre=$(echo "${file}")
  name0=$(basename ${namepre} | cut -d "." -f 1-2)
  name1=$(echo "${name0}.ufboot")
  nw_support ${file} ${curdir}/bs-file_rooted/${name1} > ${curdir}/${fdname}_bsrecal/${name0}.treefile
done
#Extract bs-recalculated trees
XtractTree ${curdir} ${fdname}_bsrecal

rm -rf ${curdir}/${fdname}_rooted/ ${curdir}/${fdname}_bsrecal/
...
#3.Scrutinize the tree and pick out the untrustworthy(at least one BS value < threshold)
#*************************************************************
echo "
         Part 3. Scrutinize the gene trees and filter out 
                      the untrustworthy gene trees"

#read -p $'**********Welcome!**********\nPlease set the threshold value of significant bootstrapping value:\n1.Default: 70.\n2.Customized value.\nChoose index: ' thva
thva=695
tree_scrutinizer ${curdir} ${thva}
rm -rf ${curdir}/gnTrees_collection.tre
...
#4.Filter out the untrustworthy gene trees and their alignments
#*************************************************************
echo "
         Part 4. Filter out the alignments of 
                            the untrustworthy gene trees"  

if [ -e output_list ]; then
   untgn="output_list" 
   untl=$(cat ${untgn} | wc -l)
   rm -rf ${curdir}/untgn_aln/
   mkdir ${curdir}/untgn_aln/

   i=1
   while [ "${i}" -le "${untl}" ]
   do
     name=$(cat ${untgn} | sed -n "${i}p" | cut -d " " -f 1)    
     for file in ${curdir}/${alnf}/*
     do
       alnnamepre=$(echo "${file}" | grep "${name}")
       if [ -n "${alnnamepre}" ]; then
          alnname=$(basename ${alnnamepre})
          mv ${alnnamepre} ${curdir}/untgn_aln/${alnname}
       fi
     done
     ((i++))
   done
...
#5.Building gene trees on the filtered alignments
#*************************************************************
echo "
        Part 5. Gene trees construction on the modified data"
...
rm -rf Trees
mkdir Trees
mv ${curdir}/${fdname}/ ${curdir}/${fdname}_rooted/ ${curdir}/gnTrees_collection.tre ${curdir}/Trees/  


fdname="iqMF-autoWZgn_1000UFB"
#Generate output layout for trees with 1000UFB
if [ -d "${curdir}/${fdname}" ]; then rm -rf ${curdir}/${fdname}; fi
mkdir ${curdir}/${fdname} ${curdir}/${fdname}/report ${curdir}/${fdname}/trivia ${curdir}/${fdname}/bs-file
...
mv ${curdir}/gnTrees_collection.tre ${curdir}/gnTrees_1000UFB_collection.tre
mv ${curdir}/${fdname}/ ${curdir}/${fdname}_rooted/ ${curdir}/gnTrees_1000UFB_collection.tre ${curdir}/Trees/
...
#6.Generate permutation data
#*************************************************************
echo "
         Part 6. Generate permutation data"
...
#7.Prepare files for permutation
#*************************************************************
echo "
         Part 7. File preparation for permutation analysis"

#Get clusters information
rm -rf ${curdir}/Analysis
mkdir ${curdir}/Analysis
mv ${curdir}/Trees/gnTrees_collection.tre ${curdir}/impMsg.txt ${curdir}/get_cluster.R ${curdir}/Analysis/
...
#9.Post permutation analysis operation
#*************************************************************
echo "
         Part 9. Post permutation analysis operation"
...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   #Assign clusters for each BS tree of the trustworthy
   #rm -rf ${curdir}/.aln_temp
   #mkdir ${curdir}/.aln_temp
   #cp -p ${curdir}/${alnf}/* ${curdir}/untgn_aln/* ${curdir}/.aln_temp/
   ...
   if [ -d "${curdir}/untgn_aln"  ]; then
      ...
      #Find the radius of each cluster
     ##Find the mean+3*SD(Radius) for each cluster
      clRdfull=1
     while [ "${clRdfull}" -le "${fnum}" ]
     do
       for alnfile in ${curdir}/${alnf}/*phy
       do
         alnname=$(basename ${alnfile} | cut -d "." -f 1 | cut -d "-" -f 2)
         clori=$(cat ${predir}/clinfo/${pknummd} | grep "${alnname}" | cut -d " " -f 2)
         i=1
         while [ "${i}" -le "${bsrep}" ]
         do
           clustind=$(cat ${curdir}/tree.Geodesic/${alnname}-cluster.assign | sed -n "${i}p")
           if [ "${clori}" -eq "${clRdfull}" ] && [ "${clustind}" -eq "${clRdfull}" ]; then
              if [ "${clustind}" -lt 10 ]; then clustindmd=$(echo "cluster0${clustind}"); else clustindmd=$(echo "cluster${clustind}"); fi

              clRdpre=$(cat ${curdir}/tree.Geodesic/${alnname}/${alnname}-${clustindmd}.geod | sed -n "${i}p")
              clRdcand=$(echo "${clRdpre} * 100000000" | bc | cut -d "." -f 1)
                     
              echo "${clRdcand}" >> ${curdir}/tree.Geodesic/${clRdfull}.geodata
           fi
           ((i++))
         done
       done

       mean=$(cat ${curdir}/tree.Geodesic/${clRdfull}.geodata | awk 'BEGIN{s=0;}{s=s+$1;}END{print s/NR;}')
       SD=$(cat ${curdir}/tree.Geodesic/${clRdfull}.geodata | awk '{sum+=$1; sumsq+=$1*$1} END {print sqrt(sumsq/NR - (sum/NR)^2)}')

       mlogic=$(echo "${mean}" | grep "e")
       if [ -n  "${mlogic}" ]; then
          num=$(echo "${mean}" | cut -d "+" -f 2 | cut -d "0" -f 2)
          numzero=$(echo "0000000000" | cut -c 1-${num})
          multiplier=$(echo "1${numzero}") 
          mean=$(echo "${mean}" | cut -d "e" -f 1) 
          meanmd=$(echo "${mean} * ${multiplier}" | bc | cut -d "." -f 1)
       else
          meanmd=${mean}
       fi

       sdlogic=$(echo "${SD}" | grep "e")
       if [ -n  "${sdlogic}" ]; then
          num=$(echo "${SD}" | cut -d "+" -f 2 | cut -d "0" -f 2)
          numzero=$(echo "0000000000" | cut -c 1-${num})
          multiplier=$(echo "1${numzero}")
          SD=$(echo "${SD}" | cut -d "e" -f 1) 
          SDmd=$(echo "${SD} * ${multiplier} * 3" | bc | cut -d "." -f 1)
       else
          SDmd=$((SD * 3))
       fi    

       echo "${meanmd} ${SDmd}"
       clRd[$clRdfull]=$((meanmd + SDmd))
       #rm -rf ${curdir}/tree.Geodesic/${clRdfull}.geodata
       ((clRdfull++))
     done      
     
     #Assign clusters for each BS tree of the untrustworthy
   ...
   ##Summarize the information of trustworthy genes
   ...
   odline=$(cat ${predir}/clinfo/dend.order | wc -l)
   odi=1
   while [ "${odi}" -le "${odline}" ]
   do
     kup=$((fnum + 1))
     gnord=$(cat ${predir}/clinfo/dend.order | sed -n "${odi}p")
     gdasn=$(echo "${curdir}/tree.Geodesic/${gnord}-cluster.assign")   
     clori=${odi}
     #clori=$(cat ${predir}/clinfo/${pknummd} | grep "${gnord}" | cut -d " " -f 2)
     if [ -z "${clori}" ]; then clori=${kup}; fi
   ...
   ##Summarize the information of untrustworthy genes
   clori=${odline}
   for gdasn in ${curdir}/tree.Geodesic/*assign
   do
     kup=$((fnum + 1))
     
     name=$(basename "${gdasn}" | cut -d "-" -f 1)   
     ((clori++))  
     #clori=$(cat ${predir}/clinfo/${pknummd} | grep "${name}" | cut -d " " -f 2)
     if [ -z "${clori}" ]; then clori=${kup}; fi
     if [ "${clori}" -lt 10 ]; then clori=$(echo "0${clori}"); fi
     name=$(echo -e "${clori}\t${name}")
     ...
     #File sorting
     mv ${gdasn} ${curdir}/tree.Geodesic/Cluster_Assignment
