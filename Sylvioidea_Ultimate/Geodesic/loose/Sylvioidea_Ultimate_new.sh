...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   if [ -d "${curdir}/untgn_aln"  ]; then
      ...
      ##Get a Geodesic distance number for each cluster to start with
      clRdfull=1
      for alnfile in ${curdir}/${alnf}/*phy
      do
        alnname=$(basename ${alnfile} | cut -d "." -f 1 | cut -d "-" -f 2)
        ...
        do
          ...
          if [ "${clRdfull}" -le "${fnum}" ]; then
             if [ "${clustind}" -eq "${clRdfull}" ]; then            
                clRd[$clRdfull]=$(echo "${clRdpre} * 100000000" | bc | cut -d "." -f 1)
               ((clRdfull++))
             fi
          fi
          ((i++))
        ...
        if [ "${clRdfull}" -gt "${fnum}" ]; then break; fi
      done
   
      ##Find the maximal number(Radius) for each cluster
      clRdfull=1
       ...
       do
         ...
         do
           ...
           if [ "${clori}" -eq "${clRdfull}" ] && [ "${clustind}" -eq "${clRdfull}" ]; then
              ...
              if [ "${clRdcand}" -gt "${clRd[$clRdfull]}" ]; then
                 clRd[$clRdfull]=${clRdcand}
              fi
           fi
           ((i++))
         done
     ...
     do
       ...
       do
         ...
         ##Compare distances to each kernal of cluster to each radius and select those that are smaller than the radius
         j=1
         clfall=0
         while [ "${j}" -le "${fnum}" ]
         do
           if [ "${gdv[$j]}" -lt "${clRd[$j]}" ]; then
              ((clfall++))
              gdvcomp[$clfall]=${gdv[$j]}
              compi[$clfall]=${j}
           fi
           ((j++))
         done
        
        ##Pick up the minimal value and index among the selected
        if [ "${clfall}" -eq 0 ]; then
           compimin=$((fnum + 1))
        else
           k=2
           gdvcompmin=${gdvcomp[1]}
           compimin=${compi[1]}
           while [ "${k}" -le "${clfall}" ]
           do
             if [ "${gdvcomp[$k]}" -lt "${gdvcompmin}" ]; then
                gdvcompmin=${gdvcomp[$k]}
                compimin=${compi[$k]}
             fi
             ((k++))
           done
         ...
         echo "${compimin}" >> ${curdir}/tree.Geodesic/${alnname}-cluster.assign
