...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   if [ -d "${curdir}/untgn_aln"  ]; then
      ...
      ##Find the average number(Radius) for each cluster
      clRdfull=1
      ...
      do
        ...
        do
          ...
          do
            ...
            if [ "${clori}" -eq "${clRdfull}" ] && [ "${clustind}" -eq "${clRdfull}" ]; then
               ...
               clRd[$clRdfull]=$((clRd[$clRdfull] + clRdcand))
               dcount[$clRdfull]=$((dcount[$clRdfull] + 1))
        ...
        clRd[$clRdfull]=$((clRd[$clRdfull] / dcount[$clRdfull]))
