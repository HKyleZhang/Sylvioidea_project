...
#read -p $'\nSpecify the directory of phyutility.jar (Default is $HOME/Software/gtp) ' phyudir
   phyudir="$HOME/Software/phyutility"
...
#Continue with ${pknum} clusters? (y/n): " pkyn
   ...
   if [ -n ${pknum} ]; then
      #Using major-rule consensus tree as supergene tree
      if [ "${pknum}" -lt 10 ]; then 
      ...
      lnum=$(cat ${predir}/clinfo/${pknummd} | wc -l)
      i=1
      while [ "${i}" -le "${lnum}" ]
      do
        trename=$(cat ${predir}/clinfo/${pknummd} | sed -n "${i}p" | cut -d " " -f 1)
        cldir=$(cat ${predir}/clinfo/${pknummd} | sed -n "${i}p" | cut -d " " -f 2)
        cat ${curdir}/Analysis/gnTrees_collection.tre | grep "${trename}" | cut -d " " -f 2 >> ${curdir}/${cldir}.tre
        ((i++))
      done

      rm -rf ${curdir}/supergn_trees
      mkdir ${curdir}/supergn_trees 

      j=1
      while [ "${j}" -le "${pknum}" ]
      do
        java -jar ${phyudir}/phyutility.jar -con -t 0.5 -in ${curdir}/${j}.tre -out ${curdir}/${j}.nextre >/dev/null
        rm -rf ${curdir}/${j}.tre *log
        java -jar ${phyudir}/phyutility.jar -vert -in ${curdir}/${j}.nextre -out ${curdir}/${j}.tre >/dev/null
        rm -rf ${curdir}/${j}.nextre *log
        if [ "${j}" -lt 10 ]; then jmd=$(echo "cluster0${j}"); else jmd=$(echo "cluster${j}"); fi
        tre=$(cat ${curdir}/${j}.tre)
        echo "${jmd} ${tre}" > ${curdir}/supergn_trees/${jmd}.tre
        rm -rf ${curdir}/${j}.tre
        ((j++))
      done
...
if [ -d "${curdir}/supergn_trees" ]; then
   cd ${curdir}
