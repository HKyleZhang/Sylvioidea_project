...
read -p "
 Files checklist: 1.Alignment folder [-a]
    ...
    Dependencies: 1.IQ-Tree           www.iqtree.org
      ...
      R packages: 1.treespace         cran.r-project.org/web/packages/treespace/index.html
                  2.heatmap3          cran.r-project.org/web/packages/heatmap3/index.html
                  3.gplots            cran.r-project.org/web/packages/gplots/index.html
                  4.ggplot2           cran.r-project.org/web/packages/ggplot2/index.html
                  5.tidyr             cran.r-project.org/web/packages/tidyr/index.html
                  6.dplyr             cran.r-project.org/web/packages/dplyr/index.html
                  7.svglite           cran.r-project.org/web/packages/svglite/index.html

Continue?(Y/n): " answer

if [ "${answer}" == "Y" -o "${answer}" == "y" ]; then
   if [ -z "${alnf}" ] || [ ! -d "${curdir}/${alnf}" ] || [ ! -e get_cluster.R ] || [ ! -e logLkhforR_make.R ] || [ ! -e graphs.R ] || [ ! -e find_potent_knum.R ] || [ ! -e clusterAssign.R ] || [ ! -e KCmetric.R ]; then echo -e "\nMsg: Some files are missing.\n" && usage && exit; fi  #Check if all the files exist 

read -p $'\nLambda = ' lambda
if [ -z "${lambda}" ]; then echo -e "\nERROR!" && exit; fi
...
for trefile in ${curdir}/${fdname}/*treefile
do
  name=$(basename ${trefile} | cut -d "." -f 1-2)  
  cat ${curdir}/bs-file/${name}.ufboot ${trefile} > ${curdir}/bs-file/${name}.ufboot-temp
  rm ${curdir}/bs-file/${name}.ufboot
  mv ${curdir}/bs-file/${name}.ufboot-temp ${curdir}/bs-file/${name}.ufboot
done
rm -rf Trees
mkdir Trees
...
cd ${curdir}/Analysis
Rscript get_cluster.R ${lambda} 2>&1 >/dev/null
rm -rf get_cluster.R
...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   mv ${curdir}/KCmetric.R ${curdir}/tree.KCmetric/
   cd ${curdir}/tree.KCmetric/
   Rscript KCmetric.R ${lambda} >/dev/null
   rm ${curdir}/tree.KCmetric/KCmetric.R
   rm -rf *tre
   for kdfile in *kcd
   do
   ...
   do
     ...
     fnum=$(ls ${curdir}/tree.KCmetric/${alnname}/ | wc -l)     
     bsreppre=$(cat ${curdir}/tree.KCmetric/${alnname}/${alnname}-cluster01.kcd | wc -l)
     bsrep=$((bsreppre - 1))
     i=1
     while [ "${i}" -le "${bsrep}" ]
     do
       ...
       do
         ...
         kcdvpre=$(cat ${curdir}/tree.KCmetric/${alnname}/${alnname}-${jmd}.kcd | sed -n "${i}p")
       ...
       echo "${min}" >> ${curdir}/tree.KCmetric/${alnname}-cluster.assign
   ...
   rm -rf ${curdir}/tree.KCmetric/Cluster_Assignment.summary ${curdir}/tree.KCmetric/Cluster_Assignment_shortdata.summary
   ...
   mv ${curdir}/clusterAssign.R ${curdir}/tree.KCmetric/
   cd ${curdir}/tree.KCmetric/
