...
do
    ...
    cat ${1}/outfile | sed -n "${startnum},${endnum}p" > ${1}/${3}_boot/boot-${filenum}.phy
    i=$((i+1))
    filenum=$((filenum+1))
done
}
...
#2.Generate the concatenation sequence
#*************************************************************
echo "
         Part 2. Generate the concatenation sequence"
...
amasmod ${curdir}/AutoWZgn_concatenation.phy
...
#3.Prepare files for bootstrapping
#*************************************************************
echo "
         Part 3. File preparation for bootstrapping analysis"
...
mv ${curdir}/gnTrees_collection.tre ${curdir}/get_cluster.R ${curdir}/Analysis/
...
if [ "${totree}" = "y" ] || [ "${totree}" = "Y" ]; then
   ...
   do
     ...
     do    
       ...
       do
         amasmod ${seqfile}
         fseqboot -sequence ${seqfile} -outfile outfile -test b -seqtype d -rewriteformat p -reps 100 > /dev/null
         seqname=$(echo "${seqfile}" | cut -d "." -f 1 | cut -d "_" -f 3 | cut -d "-" -f 1)
         taxanum=6
         makedatasets ${tempdir} ${taxanum} ${seqname} 
         rm -rf outfile
         mv ${seqfile} ${tempdir}/${seqname}_boot/boot-10.phy

         for bsfile in ${tempdir}/${seqname}_boot/*phy
         do
           iqtree -s ${bsfile} -m GTR+F+I -AICc -ninit 200 -ntop 50 -nt ${nth} -keep-ident > /dev/null
           logvpre=$(cat ${tempdir}/${seqname}_boot/*iqtree | grep "Log-likelihood of the tree:" | cut -d " " -f 5)
           logv=$(echo "${logvpre} * 10000" | bc | cut -d "." -f 1 | cut -d "-" -f 2)
           echo "${logv}" >> ${tempdir}/${kclnum}_${seqname}.logval
           rm -rf ${tempdir}/${seqname}_boot/*iqtree
         done
         rm -rf ${tempdir}/${seqname}_boot
       done
       
       filenum=$(ls | wc -l)
       lnum=$(cat ${tempdir}/${kclnum}_cluster01.logval | wc -l)
       
       ##Calculate the sum log likelihood for the k number of clusters
       l=1
       while [ "${l}" -le "${lnum}" ]
       do
         f=1
         ...
         while [ "${f}" -le "${filenum}" ]
         do
           if [ "${f}" -lt 10 ]; then fmd=$(echo "0${f}"); else fmd=${f}; fi
           valpre=$(cat ${kclnum}_cluster${fmd}.logval | sed -n "${l}p")
           val=$((val + valpre))
           ((f++))
         done
         echo "${val}" >> ${predir}/logLikelihood/${kclnum}.logLkh
         ((l++))
       done
