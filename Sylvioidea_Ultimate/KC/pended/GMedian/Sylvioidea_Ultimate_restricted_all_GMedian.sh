...
do
  ...
  alnname=$(basename ${alnfile})
  name=$(basename ${alnfile} | cut -d "." -f 1 | cut -d "-" -f 2)
  logvalue=$(cat ${curdir}/${fdname}/report/${alnname}.iqtree | grep "Log-likelihood of consensus tree:" | cut -d " " -f 5 | cut -d "-" -f 2)
  log=$(echo "${logvalue} * 10000" | bc | cut -d "." -f 1)
  echo -e "${name}\t${log}" >> ${curdir}/tree.logLkh  
done
...
fseqboot -sequence ${curdir}/AutoWZgn_concatenation.phy -outfile outfile -test o -seqtype d -rewriteformat p -reps 10 -seed ${seednum} > /dev/null
...
rm -rf ${curdir}/AutoWZgn_concatenation.phy
...
do
  ...
  mkdir ${name[$ni]}
  python ${amasdir}/AMAS.py split -f phylip-int -d dna -i ${file} -l ${predir}/partition -u phylip > /dev/null 

  for kfile in ${predir}/clinfo/*cl
  do
    kname=$(basename ${kfile} | cut -d "." -f 1)    
    mkdir ${predir}/${name[$ni]}/${kname}
    kup=$(echo "${kname}" | cut -d "k" -f 2 | cut -d "0" -f 2)
    ki=1
    while [ "${ki}" -le "${kup}" ]
    do
      if [ "${ki}" -lt 10 ]; then kmd=$(echo "0${ki}"); else kmd=${ki}; fi
      mkdir ${predir}/${name[$ni]}/${kname}/${kmd}
      ((ki++))
    done
    ...
    while [ "${li}" -le "${lnum}" ]
    do
      gnname=$(cat ${kfile} | sed -n "${li}p" | cut -d " " -f 1)
      cl=$(cat ${kfile} | sed -n "${li}p" | cut -d " " -f 2)
      if [ "${cl}" -lt 10 ]; then clsort=$(echo "0${cl}"); else clsort=${cl}; fi
      target=$(ls ${predir} | grep "${gnname}" | grep "${name[$ni]}")      
      cp -p ${predir}/${target} ${predir}/${name[$ni]}/${kname}/${clsort}/${gnname}.phy
      ((li++))
    done
  done
done
...
if [ "${totree}" = "y" ] || [ "${totree}" = "Y" ]; then
   ...
   do
     ...
     do
       ...
       do
         if [ "${ki}" -lt 10 ]; then kmd=$(echo "0${ki}"); else kmd=${ki}; fi
         cd ${predir}/${name[$tri]}/${kname}/${kmd}

         tempdir=$(pwd)
         filenum=$(ls | wc -l) #Count file number as multiplier for the sum log likelihood
         ##Build tree 
         for seqfile in *phy
         do         
           ...
           iqtree -s ${seqfile} -m GTR+F+I -AICc -ninit 200 -ntop 50 -nt ${nth} -keep-ident > /dev/null

           rm -rf ${seqfile}  #For saving the storage room
         done
         
         rm -rf *bionj *ckp.gz *log *mldist #Remove irrelevant files
set -x
         ##Collect log likelihood value
         for iqfile in *iqtree
         do
           name=$(echo "${iqfile}" | cut -d "." -f 1)
           logv=$(cat ${iqfile} | grep "Log-likelihood of the tree:" | cut -d " " -f 5 | cut -d "-" -f 2)
           log=$(echo "${logv} * 10000" | bc | cut -d "." -f 1)
           echo "${name} ${log}" >> logLkh_collect.val
         done
         rm -rf *iqtree
 
         ##Root the tree
         for tfile in *treefile
         do
           name=$(echo ${tfile} | cut -d "." -f 1)
           nw_reroot ${tfile} ZF > ${name}.rootree
         done
         rm -rf *treefile

         ##Collect trees
         for tfile in *rootree
         do
           name=$(echo ${tfile} | cut -d "." -f 1)
           tre=$(cat ${tfile})
           echo "${name} ${tre}" >> tree_collect.tre
         done
         rm -rf *rootree

         ##Find the median tree
         cp -p ${curdir}/GMedian.R ${tempdir}
         Rscript GMedian.R tree_collect.tre
         rm -rf GMedian.R

         if [ -e Median.tre ]; then
            medianname=$(cat Median.tre)
            medlogpre=$(cat logLkh_collect.val | grep "${medianname}" | cut -d " " -f 2)
            medlog[$ki]=$(echo "${medlogpre} * ${filenum}" | bc | cut -d "." -f 1)
         fi
         ((ki++))
       done 
       ...
       while [ "${ki}" -le "${kup}" ]
       do
         medlogsum=$((medlogsum + medlog[$ki]))
         ((ki++))
       done
       echo -e "${name[$tri]} ${kname} -loglikelihood*10^4:\t${medlogsum}" >> ${predir}/logLikelihood/${name[$tri]}_logLhcollection
set +x
     done
     ((tri++))
   done
...
if [ -e ${curdir}/gnTrees_collection.tre ]; then
   ...
   #Generate the tree collection file to calculate KCmetrics distance
   rm -rf tree.KCmetrics
   mkdir tree.KCmetrics
   ...
   do
     ...
     do
       ...
       cat ${sgt} ${bst} > ${curdir}/tree.KCmetrics/${bstname}-${sgtname}.tre
     done
   done
   
   #Calculate the KCmetrics distances between each supergene tree and the BS trees    
   mv ${curdir}/KCmetrics.R ${curdir}/tree.KCmetrics/
   cd ${curdir}/tree.KCmetrics/
   Rscript KCmetrics.R >/dev/null
   rm ${curdir}/tree.KCmetrics/KCmetrics.R
   rm -rf *tre
   for kdfile in *kcd
   do
     clsort=$(echo "${kdfile}" | cut -d "-" -f 1)
     if [ ! -d "${curdir}/tree.KCmetrics/${clsort}" ]; then mkdir ${curdir}/tree.KCmetrics/${clsort}; fi
     mv ${kdfile} ./${clsort}/
   done
   ...
   do
     ...
     fnum=$(ls ${curdir}/tree.KCmetrics/${alnname}/ | wc -l)     
     bsrep=$(cat ${curdir}/tree.KCmetrics/${alnname}/${alnname}-cluster01.kcd | wc -l)
     i=1
     while [ "${i}" -le "${bsrep}" ]
     do
       j=1
       ...
       do
         ...
         kcdvpre=$(cat ${curdir}/tree.KCmetrics/${alnname}/${alnname}-${jmd}.kcd | sed -n "${i}p")
       ...
       echo "${min}" >> ${curdir}/tree.KCmetrics/${alnname}-cluster.assign
   ...
   rm -rf ${curdir}/tree.KCmetrics/Cluster_Assignment
   mkdir ${curdir}/tree.KCmetrics/Cluster_Assignment
   
   rm -rf ${curdir}/tree.KCmetrics/Cluster_Assignment.summary ${curdir}/tree.KCmetrics/Cluster_Assignment_shortdata.summary
   ...
   do
     ...
     kcdasn=$(echo "${curdir}/tree.KCmetrics/${gnord}-cluster.assign")   
     clori=${odi}
     ...
     echo -e "${name}\t${ksum}" >> ${curdir}/tree.KCmetrics/Cluster_Assignment_shortdata.summary        

     #Produce long-data table output
     l=1
     while [ "${l}" -le "${fnum}" ]
     do
       echo -e "${name}\tcluster${l}\t${kcount[$l]}" >> ${curdir}/tree.KCmetrics/Cluster_Assignment.summary
       ((l++))
     done
     
     #File sorting
     mv ${curdir}/tree.KCmetrics/${gnord}-cluster.assign ${curdir}/tree.KCmetrics/Cluster_Assignment

     ((odi++))
   done

   #File sorting   
   rm -rf ${curdir}/tree.KCmetrics/KCmetrics_distance
   mkdir ${curdir}/tree.KCmetrics/KCmetrics_distance
   for alnfile in ${curdir}/${alnf}/*phy
   do
     alnname=$(basename ${alnfile} | cut -d "." -f 1 | cut -d "-" -f 2)
     mv ${curdir}/tree.KCmetrics/${alnname} ${curdir}/tree.KCmetrics/KCmetrics_distance/
   done
   
   #Plot graphs for cluster assignment
   mv ${curdir}/clusterAssign.R ${curdir}/tree.KCmetrics/
   cd ${curdir}/tree.KCmetrics/
